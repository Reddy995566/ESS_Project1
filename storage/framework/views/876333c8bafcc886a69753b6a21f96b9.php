...
<?php $__env->startSection('step_description', 'Add color, size, and stock variants for your product'); ?>
...
<form id="stepForm" action="<?php echo e(route('seller.products.create.step2.process')); ?>" method="POST" enctype="multipart/form-data">
    <?php echo csrf_field(); ?>
...
const availableColors = <?php echo json_encode($colors, 15, 512) ?>;
const availableSizes = <?php echo json_encode($sizes, 15, 512) ?>;
...
// Initialize - Load from session if available
document.addEventListener('DOMContentLoaded', function() {
    // Load from old input (validation errors) or session
    <?php if(old('variants_data')): ?>
        try {
            const oldVariants = JSON.parse('<?php echo addslashes(old('variants_data')); ?>');
            if (Array.isArray(oldVariants) && oldVariants.length > 0) {
                oldVariants.forEach(v => addVariant(v));
            }
        } catch(e) {
            console.error('Failed to load old variants:', e);
        }
    <?php elseif(session('seller_product_data.variants_data')): ?>
        try {
            const sessionVariants = <?php echo json_encode(json_decode(session('seller_product_data.variants_data'), true), 512) ?>;
            if (Array.isArray(sessionVariants) && sessionVariants.length > 0) {
                sessionVariants.forEach(v => addVariant(v));
            }
        } catch(e) {
            console.error('Failed to load session variants:', e);
        }
    <?php endif; ?>
...
<?php echo $__env->make('seller.products.create._layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ESS_Project1\resources\views/seller/products/create/step2.blade.php ENDPATH**/ ?>