...
<?php $__env->startSection('title', 'Approved Products'); ?>

<?php $__env->startSection('content'); ?>
<!-- Main Container -->
<div class="min-h-screen bg-gradient-to-br from-gray-50 via-green-50 to-emerald-50 p-6">
    ...
    <div class="mb-8">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-3xl font-black text-gray-900 tracking-tight">Approved Products</h1>
                    <p class="mt-1 text-sm text-gray-600">Products that have been approved and are live</p>
                </div>
            </div>
            <div class="flex items-center space-x-3">
                <button id="refreshBtn" class="inline-flex items-center justify-center w-11 h-11 bg-white text-gray-700 rounded-xl border-2 border-gray-200 hover:border-green-400 hover:bg-green-50 hover:text-green-700 transition-all duration-200 shadow-sm hover:shadow-md">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
            ...
            <div class="text-white">
                <p class="text-4xl font-black mb-1"><?php echo e($products->filter(function($product) { 
                    if (!$product->approved_at) return false;
                    $approvedDate = is_string($product->approved_at) ? \Carbon\Carbon::parse($product->approved_at) : $product->approved_at;
                    return $approvedDate->isToday();
                })->count()); ?></p>
                <p class="text-blue-100 text-sm font-medium">Today's Approvals</p>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
            ...
            <div class="text-white">
                <p class="text-4xl font-black mb-1"><?php echo e($products->filter(function($product) { 
                    if (!$product->approved_at) return false;
                    $approvedDate = is_string($product->approved_at) ? \Carbon\Carbon::parse($product->approved_at) : $product->approved_at;
                    return $approvedDate->isCurrentWeek();
                })->count()); ?></p>
        ...
        <div class="bg-gradient-to-br from-yellow-500 to-orange-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
            ...
            <div class="text-white">
                <p class="text-4xl font-black mb-1"><?php echo e($products->where('status', 'active')->count()); ?></p>
                <p class="text-yellow-100 text-sm font-medium">Active Products</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-gray-50 to-slate-50 px-6 py-5 border-b border-gray-200">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1 ml-11">Products that have been approved and are live on the website</p>
                </div>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table id="approvedProductsTable" class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-slate-100 to-gray-100">
                    <tr>
                        ...
                        <th class="px-4 py-4 text-center w-32">
                            <span class="text-xs font-black text-gray-700 uppercase">Approved</span>
                        </th>
                        <th class="px-4 py-4 text-center w-32">
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center space-x-2 mt-2">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 border border-green-300">
                                            ✅ Approved
                                        </span>
                        ...
                        <td class="px-4 py-4 text-center">
                            <?php if($product->approved_at): ?>
                                <?php
                                    $approvedDate = is_string($product->approved_at) ? \Carbon\Carbon::parse($product->approved_at) : $product->approved_at;
                                ?>
                                <div class="text-sm font-medium text-gray-900"><?php echo e($approvedDate->format('M d, Y')); ?></div>
                                <div class="text-xs text-gray-500"><?php echo e($approvedDate->diffForHumans()); ?></div>
                            <?php else: ?>
                    ...
                    <tr id="emptyState">
                        <td colspan="9" class="px-6 py-20">
                            <div class="text-center">
                                <div class="mx-auto flex items-center justify-center w-24 h-24 bg-gradient-to-br from-green-100 to-emerald-200 rounded-full mb-6">
                                    <svg class="w-12 h-12 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                    </svg>
                                </div>
                                <h3 class="text-xl font-bold text-gray-900 mb-2">No Approved Products</h3>
                                <p class="text-gray-500 mb-6 max-w-md mx-auto">No products have been approved yet.</p>
                            </div>
                        </td>
                    </tr>
                    <?php endif; ?>
        ...
        <div class="bg-gradient-to-r from-gray-50 to-slate-50 px-6 py-4 border-t border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="dataTables_info text-sm font-medium text-gray-700">
                    Showing <span class="font-bold text-green-600"><?php echo e($products->firstItem() ?? 0); ?></span> to <span class="font-bold text-green-600"><?php echo e($products->lastItem() ?? 0); ?></span> of <span class="font-bold text-green-600"><?php echo e($products->total()); ?></span> entries
                </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div class="flex items-center space-x-2 mb-4">
                                        ...
                                        ${response.is_featured ? '<span class="px-3 py-1 bg-yellow-100 text-yellow-800 text-sm font-medium rounded-full">⭐ Featured</span>' : ''}
                                    </div>
                                </div>
                                ...
                                <div class="bg-green-50 p-3 rounded-lg border border-green-200">
                                    <p class="text-xs font-semibold text-green-600 uppercase">Approved Date</p>
                                    <p class="text-sm font-bold text-green-700">${response.approved_at || 'N/A'}</p>
                                </div>
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ESS_Project1\resources\views/admin/sellers/approved-products.blade.php ENDPATH**/ ?>