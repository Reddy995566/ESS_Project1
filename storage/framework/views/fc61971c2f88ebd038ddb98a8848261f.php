<?php $__env->startSection('step_title', 'Step 5: SEO Optimization'); ?>
<?php $__env->startSection('step_description', 'Optimize for search engines and social media'); ?>

<?php $__env->startSection('step_content'); ?>
<?php
    $currentStep = 5;
    $prevStepRoute = route('seller.products.create.step4');
?>

<form id="stepForm" action="<?php echo e(route('seller.products.create.step5.process')); ?>" method="POST">
    <?php echo csrf_field(); ?>
        ...
        <div class="p-8 space-y-6">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                    ...
                    <button type="button" onclick="autoGenerateSEO()" class="px-4 py-2 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white text-sm font-semibold rounded-lg shadow-md hover:from-indigo-600 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 flex items-center space-x-2">
                        <span>✨</span>
...
function autoGenerateSEO() {
    // Data from server
    const productName = <?php echo json_encode($productData['name'] ?? '', 15, 512) ?>;
    const productDesc = <?php echo json_encode($productData['description'] ?? '', 15, 512) ?>;
    const brandName = <?php echo json_encode($brand->name ?? '', 15, 512) ?>;
    const categoryName = <?php echo json_encode($category->name ?? '', 15, 512) ?>;
    const price = <?php echo json_encode($productData['price'] ?? '', 15, 512) ?>;
    ...
    const productSlug = <?php echo json_encode($productData['slug'] ?? 'product', 15, 512) ?>;
    ...
    // Trigger input events to updatecounters and preview
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('prevBtn')?.addEventListener('click', function() {
        window.location.href = '<?php echo e(route("seller.products.create.step5")); ?>';
    });
});
</script>
<?php $__env->stopPush(); ?>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('seller.products.create._layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ESS_Project1\resources\views/seller/products/create/step6.blade.php ENDPATH**/ ?>