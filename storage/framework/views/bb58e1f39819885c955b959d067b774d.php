<?php $__env->startSection('step_title', 'Step 2: Product Variants'); ?>
<?php $__env->startSection('step_description', 'Configure colors, sizes and variant-specific options'); ?>

<?php $__env->startSection('step_content'); ?>
<?php
    $currentStep = 2;
    $prevStepRoute = route('seller.products.create.step1');
?>

<form id="stepForm" action="<?php echo e(route('seller.products.create.step2.process')); ?>" method="POST">
    <?php echo csrf_field(); ?>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-200">
        ...
        <div class="p-8 space-y-8">
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-3 md:grid-cols-6 lg:grid-cols-8 gap-4" id="colorsGrid">
                        ...
                        <label class="cursor-pointer group">
                            <input type="checkbox" name="selected_colors[]" value="<?php echo e($color->id); ?>" class="hidden color-checkbox" data-color-id="<?php echo e($color->id); ?>" data-color-name="<?php echo e($color->name); ?>" data-color-hex="<?php echo e($color->hex_code); ?>">
                            <div class="relative">
                            ...
                            <div class="text-xs font-medium text-center mt-2 text-gray-700"><?php echo e($color->name); ?></div>
                        </label>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-4 md:grid-cols-8 lg:grid-cols-12 gap-3">
                        ...
                        <label class="cursor-pointer">
                            <input type="checkbox" name="selected_sizes[]" value="<?php echo e($size->id); ?>" class="hidden size-checkbox" data-size-id="<?php echo e($size->id); ?>" data-size-name="<?php echo e($size->name); ?>" data-size-abbr="<?php echo e($size->abbreviation); ?>">
                            <div class="relative">
                                <div class="w-16 h-12 rounded-lg border-2 border-gray-300 transition-all duration-200 flex items-center justify-center font-bold text-lg bg-white text-gray-700 hover:border-indigo-300 size-box">
                                    <?php echo e($size->abbreviation ?? substr($size->name, 0, 2)); ?>
                            ...
                            <div class="text-xs font-medium text-center mt-1 text-gray-700"><?php echo e($size->name); ?></div>
                        </label>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                </div>
            ...
            <input type="hidden" name="has_variants" id="hasVariantsInput" value="<?php echo e(old('has_variants', isset($productData['has_variants']) && $productData['has_variants'] ? '1' : '0')); ?>">
            <input type="hidden" name="variant_colors" id="variantColorsInput" value="<?php echo e(old('variant_colors', json_encode($productData['variant_colors'] ?? []))); ?>">
            <input type="hidden" name="variant_sizes" id="variantSizesInput" value="<?php echo e(old('variant_sizes', json_encode($productData['variant_sizes'] ?? []))); ?>">
            <input type="hidden" name="variant_images" id="variantImagesInput" value="<?php echo e(old('variant_images', json_encode($productData['variant_images'] ?? []))); ?>">
        </div>
    </div>
</form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function handleUpload(fileList, colorId, colorName) {
        ...
        const uploadPromises = files.map(async (file) => {
            ...
            try {
                const response = await fetch('<?php echo e(route("seller.upload.image")); ?>', {
                    method: 'POST',
                    body: formData,
    ...
    document.getElementById('nextBtn')?.addEventListener('click', async function(e) {
        ...
        console.log('Create Form submission - Hidden input values:');
        console.log('variant_colors:', document.getElementById('variantColorsInput').value);
        console.log('variant_sizes:', document.getElementById('variantSizesInput').value);
        console.log('variant_images:', document.getElementById('variantImagesInput').value);
        ...
        try {
            ...
            if (result.success) {
                window.location.href = result.next_step_url || '<?php echo e(route("seller.products.create.step3")); ?>';
            } else {
...
<?php echo $__env->make('seller.products.create._layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ESS_Project1\resources\views/seller/products/create/step5.blade.php ENDPATH**/ ?>