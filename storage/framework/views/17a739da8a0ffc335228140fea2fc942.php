...
<script>
    ...
    function resetForm() {
        document.getElementById('modalTitle').textContent = 'Add New fabric';
        document.getElementById('submitBtnText').textContent = 'Create fabric';
        document.getElementById('fabricForm').action = '<?php echo e(route('seller.fabrics.store')); ?>';
        ...
        document.querySelector('input[name="show_in_navbar"]').checked = false;
        document.querySelector('input[name="show_in_homepage"]').checked = false;
    }
    ...
    function openEditModal(data) {
        document.getElementById('modalTitle').textContent = 'Edit fabric';
        document.getElementById('submitBtnText').textContent = 'Update fabric';
        document.getElementById('fabricForm').action = '/admin/fabrics/' + data.id;
        document.getElementById('methodField').value = 'PATCH';
        document.getElementById('fabricId').value = data.id;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('fillUpBtn')?.addEventListener('click', function() {
            ...
            const randomfabric = samplefabrics[Math.floor(Math.random() * samplefabrics.length)];
            
            document.getElementById('name').value = randomfabric.name;
            document.querySelector('input[name="sort_order"]').value = Math.floor(Math.random() * 100);
            document.querySelector('input[name="is_active"]').checked = true;
            document.querySelector('input[name="show_in_navbar"]').checked = Math.random() < 0.5;
            document.querySelector('input[name="show_in_homepage"]').checked = Math.random() < 0.5;
        ...
        document.querySelectorAll('.delete-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                const fabricId = this.dataset.id;
                const fabricName = this.dataset.name;
                const deleteBtn = this;

                showConfirmDialog(
                    'Delete fabric?',
                    `Are you sure you want to delete "${fabricName}"? This action cannot be undone.`,
                    () => {
                ...
                .then(data => {
                    if (data.success) {
                        showToastNotification('fabric deleted successfully!', 'success');
                        const row = deleteBtn.closest('tr');
                        const fabricData = {
                            is_active: row.dataset.status === '1'
                        };

                        updateStatsOnDelete(fabricData);
                        ...
                        showToastNotification(data.message || 'Failed to delete fabric', 'error');
        ...
        document.querySelectorAll('.status-toggle').forEach(toggle => {
            toggle.addEventListener('change', function() {
                ...
                fetch('/admin/fabrics/' + fabricId + '/toggle', {
                    method: 'POST',
                    headers: {
        ...
        document.getElementById('fabricForm')?.addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (methodField === 'POST') {
                        updateStatsOnCreate(data.fabric);
                        addfabricRow(data.fabric);
                    } else {
                        const oldRow = document.querySelector(`#fabricsTable tbody tr[data-id="${data.fabric.id}"]`);
                        if (oldRow) {
                            const oldData = {
                                isActive: oldRow.dataset.status === '1'
                            };
                            updateStatsOnEdit(data.fabric, oldData);
                            updatefabricRow(data.fabric);
                        }
                    }
                } else {
                    if (data.errors) {
                        ...
                        showToastNotification(data.message || 'Failed to save fabric', 'error');
                    }
                    submitBtn.disabled = false;
                    spinner?.classList.add('hidden');
                }
        ...
        document.getElementById('exportBtn')?.addEventListener('click', function() {
            window.open('/admin/fabrics/export', '_blank');
        ...
        document.getElementById('applyBulkAction')?.addEventListener('click', function() {
            ...
            if (action === 'delete') {
                showConfirmDialog(
                    'Delete Multiple fabrics?',
                    `Are you sure you want to delete ${selected.length} selected fabric${selected.length > 1 ? 's' : ''}? This action cannot be undone.`,
                    () => {
        ...
        function performBulkAction(action, selected) {
                ...
                fetch('/admin/fabrics/bulk-action', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    ...
    function updateStatsOnCreate(fabric) {
        // Total fabrics
    ...
    function updateStatsOnEdit(newfabric, oldData) {
        // Active status changed?
        if (newfabric.is_active !== oldData.isActive) {
            const activeEl = document.querySelector('.bg-gradient-to-br.from-green-500 .text-4xl');
            if (activeEl) {
                let count = parseInt(activeEl.textContent);
                activeEl.textContent = newfabric.is_active ? count + 1 : count - 1;
            }
        }
    }
    ...
    function updateStatsOnDelete(fabric) {
        // Total fabrics
        const totalEl = document.querySelector('.bg-gradient-to-br.from-blue-500 .text-4xl');
        if (totalEl) {
    ...
    function addfabricRow(fabric) {
        const tbody = document.querySelector('#fabricsTable tbody');
        if (!tbody) return;
        ...
        const row = createfabricRow(fabric);
    ...
    function updatefabricRow(fabric) {
        const row = document.querySelector(`#fabricsTable tbody tr[data-id="${fabric.id}"]`);
        if (!row) return;

        const newRow = createfabricRow(fabric);
    ...
    function createfabricRow(fabric) {
        ...
        row.innerHTML = `
            ...
            <td class="px-4 py-4">
                <p class="text-sm font-bold text-gray-900 group-hover:text-blue-700">${escapeHtml(fabric.name)}</p>
            </td>
            <td class="px-4 py-4 text-center">
                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-700 border border-blue-300">${fabric.sort_order}</span>
            </td>
            <td class="px-4 py-4 text-center">
            ...
            <td class="px-4 py-4">
                <div class="flex items-center justify-center space-x-2">
                    <button class="edit-btn inline-flex items-center justify-center w-9 h-9 text-blue-600 bg-blue-50 border-2 border-blue-200 rounded-lg hover:bg-blue-100 hover:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
                        title="Edit fabric"
                        data-id="${fabric.id}"
                        data-name="${escapeHtml(fabric.name)}"
...
<?php /**PATH C:\xampp\htdocs\ESS_Project1\resources\views/seller/fabrics/scripts.blade.php ENDPATH**/ ?>