<?php $__env->startSection('title', 'Shop By Budget Cards'); ?>
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-zinc-100 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" id="budget-cards-container">
            <?php $__empty_1 = true; $__currentLoopData = $budgetCards; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $card): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden group" data-id="<?php echo e($card->id); ?>">
                <!-- Preview Card -->
                <div class="relative h-48 flex items-center justify-center" style="background: linear-gradient(to bottom right, var(--color-primary, #5C1F33), var(--color-primary-light, #7A2D45), var(--color-primary, #5C1F33));">
                    <div class="w-32 h-32 flex items-center justify-center rotate-45 border-2" style="border-color: var(--color-accent-gold, #E6B873);">
                        <div class="flex flex-col items-center justify-center -rotate-45">
                            <p class="text-white uppercase font-semibold text-sm"><?php echo e($card->title); ?></p>
                            <p class="text-white uppercase font-medium text-xs mt-1"><?php echo e($card->subtitle); ?></p>
                            <p class="font-bold text-xl mt-1" style="color: var(--color-accent-gold, #E6B873);">₹ <?php echo e(number_format($card->price)); ?></p>
                        </div>
                    </div>
                    
                    <!-- Status Badge -->
                    <div class="absolute top-3 right-3">
                        <span class="px-2 py-1 text-xs font-medium rounded-full <?php echo e($card->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'); ?>">
                ...
                <div class="p-4">
                    <div class="flex items-center justify-between mb-3">
                        <div>
                            <h3 class="font-semibold text-gray-900"><?php echo e($card->title); ?> <?php echo e($card->subtitle); ?> ₹<?php echo e(number_format($card->price)); ?></h3>
                            <p class="text-xs text-gray-500">Sort: <?php echo e($card->sort_order); ?></p>
                        </div>
                    </div>
                    
                    <!-- Actions -->
                    <div class="flex items-center gap-2">
                        <button onclick="editCard(<?php echo e($card->id); ?>, '<?php echo e($card->title); ?>', '<?php echo e($card->subtitle); ?>', <?php echo e($card->price); ?>, '<?php echo e($card->link); ?>', <?php echo e($card->sort_order); ?>, <?php echo e($card->is_active ? 'true' : 'false'); ?>)" 
                            class="flex-1 px-3 py-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition text-sm font-medium">
                            Edit
                        </button>
                        <button onclick="toggleStatus(<?php echo e($card->id); ?>)" 
                            class="px-3 py-2 <?php echo e($card->is_active ? 'bg-orange-50 text-orange-600 hover:bg-orange-100' : 'bg-green-50 text-green-600 hover:bg-green-100'); ?> rounded-lg transition text-sm font-medium">
                            <?php echo e($card->is_active ? 'Disable' : 'Enable'); ?>

                        </button>
                        <button onclick="deleteCard(<?php echo e($card->id); ?>)" 
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ESS_Project1\resources\views/admin/budget-cards/index.blade.php ENDPATH**/ ?>