...
<?php $__env->startSection('title', 'Edit Product - Step <?php echo e($step ?? 1); ?> of 6'); ?>
        ...
        <div class="w-full px-6 py-5">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <?php endif; ?>
                    <a href="<?php echo e(route('seller.products.index')); ?>" class="px-5 py-2.5 border-2 border-red-300 text-red-700 bg-white hover:bg-red-50 hover:border-red-400 rounded-lg font-semibold transition-all duration-200 hover:shadow-sm">
                        Cancel Edit
                    </a>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <div class="flex items-center justify-center">
                    <div class="flex items-center space-x-4 sm:space-x-6 lg:space-x-8 xl:space-x-10">
                        ...
                        <?php $__currentLoopData = $steps; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $stepItem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <!-- Step <?php echo e($stepItem['number']); ?> -->
                            <div class="flex flex-col items-center space-y-1 sm:space-y-2 cursor-pointer group step-indicator <?php echo e($stepItem['number'] == $current ? 'active' : ''); ?> <?php echo e($stepItem['number'] < $current ? 'completed' : ''); ?>" 
                                 onclick="navigateToStep(<?php echo e($stepItem['number']); ?>)" title="Click to go to <?php echo e($stepItem['title']); ?>">
                                <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-xs sm:text-sm font-bold shadow-lg group-hover:scale-110 group-hover:shadow-xl transition-all duration-200
                                    <?php echo e($stepItem['number'] == $current ? 'bg-gradient-to-r from-indigo-600 to-indigo-700 text-white ring-4 ring-indigo-200' : ''); ?>

                                    <?php echo e($stepItem['number'] < $current ? 'bg-gradient-to-r from-green-600 to-green-700 text-white' : ''); ?>

                                    <?php echo e($stepItem['number'] > $current ? 'bg-gradient-to-r from-gray-400 to-gray-500 text-white hover:from-indigo-500 hover:to-indigo-600' : ''); ?>">
                                    <?php echo e($stepItem['number'] < $current ? '✓' : $stepItem['number']); ?>

                                </div>
                                <div class="text-center">
                                    <div class="text-xs sm:text-sm font-semibold <?php echo e($stepItem['number'] == $current ? 'text-gray-900' : ($stepItem['number'] < $current ? 'text-green-700' : 'text-gray-700 group-hover:text-indigo-900')); ?>"><?php echo e($stepItem['title']); ?></div>
                                    <div class="text-xs <?php echo e($stepItem['number'] == $current ? 'text-gray-500' : ($stepItem['number'] < $current ? 'text-green-500' : 'text-gray-500 group-hover:text-indigo-700')); ?> hidden sm:block"><?php echo e($stepItem['description']); ?></div>
                                </div>
                            </div>

                            <?php if($index < count($steps) - 1): ?>
                                <div class="w-8 sm:w-12 lg:w-16 h-0.5 rounded-full progress-line <?php echo e($stepItem['number'] < $current ? 'active bg-gradient-to-r from-green-500 to-green-600' : 'bg-gray-300'); ?>"></div>
                            <?php endif; ?>
    ...
    <div class="w-full px-6 py-6">
        ...
        <div class="mt-8 flex justify-between items-center">
            ...
            <div class="flex space-x-3">
                <button type="button" id="updateBtn" class="px-8 py-3 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white rounded-lg font-semibold transition-all duration-200 shadow-lg hover:shadow-xl">
                    ✅ Update Step
                </button>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // NOTE: Update button handler is in individual step files
});
...
function navigateToStep(stepNumber) {
    ...
    window.location.href = '/seller/products/' + productId + '/edit?step=' + stepNumber;
}
...
<?php echo $__env->make('seller.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ESS_Project1\resources\views/seller/products/edit/_layout.blade.php ENDPATH**/ ?>