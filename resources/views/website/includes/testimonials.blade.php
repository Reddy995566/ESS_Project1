            ...
            <div class="swiper-slide h-auto">
                <div class="bg-transparent md:rounded-md p-5 text-center flex flex-col items-center h-full mx-0 md:mx-0" style="border: 1px solid var(--color-primary);">
                    <div class="flex gap-1 mb-3 justify-center">
                        @for($i=0; $i<5; $i++)
                            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24" style="color: {{ $i < $testimonial->rating ? 'var(--color-accent-gold)' : '#d1d5db' }};"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                        @endfor
                    </div>
                    <p class="text-[13px] italic leading-relaxed mb-4" style="color: var(--color-primary);">
                        "{{ $testimonial->review }}"
                    </p>
                    <p class="text-[14px] font-medium mt-auto" style="color: var(--color-primary);">— {{ $testimonial->name }}</p>
                </div>
            </div>
            @endforeach
        </div>
