...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateSelectedColorsTable() {
        console.log('updateSelectedColorsTable called, selectedColors:', selectedColors);
        const section = document.getElementById('selectedColorsSection');
        const container = document.getElementById('selectedColorsInputs');
        
        console.log('section:', section, 'container:', container);
        
        if (selectedColors.length === 0) {
            section.style.display = 'none';
            console.log('No colors selected, hiding section');
            return;
        }
        
        console.log('Showing section with', selectedColors.length, 'colors');
        ...
        selectedColors.forEach(colorId => {
            const checkbox = document.querySelector(`.color-checkbox[data-color-id="${colorId}"]`);
            if (!checkbox) {
                console.log('Checkbox not found for colorId:', colorId);
                return;
            }
            ...
            colorDiv.className = 'flex items-center gap-4 p-4 bg-gray-50 rounded-lg border border-gray-200';
            colorDiv.innerHTML = `
                <div class="flex items-center gap-3 flex-1">
                    <div class="w-12 h-12 rounded-lg border-2 border-gray-300 shadow-sm" style="background-color: ${colorHex};"></div>
                    <span class="text-sm font-semibold text-gray-900">${colorName}</span>
                </div>
                <div class="flex items-center gap-2">
                    <label class="text-sm font-medium text-gray-700">Stock Qty:</label>
                    <input type="number" 
                           name="color_stock[${colorId}]" 
                           min="0" 
                           value="10"
                           placeholder="Qty"
                           class="w-24 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-sm font-medium">
                    <span class="text-xs text-gray-500">units</span>
                </div>
                <button type="button" 
                        onclick="removeColorFromList('${colorId}')"
    ...
    // Global function to remove color from list
    window.removeColorFromList = function(colorId) {
        selectedColors = selectedColors.filter(id => id !== colorId);
        
        const checkbox = document.querySelector(`.color-checkbox[data-color-id="${colorId}"]`);
        if (checkbox) checkbox.checked = false;
        
        if (colorImages[colorId]) delete colorImages[colorId];
    ...
    // Initial Render - ONLY show selected colors list, NOT image containers
    if(selectedColors.length > 0) {
        updateSelectedColorsTable();
    }
