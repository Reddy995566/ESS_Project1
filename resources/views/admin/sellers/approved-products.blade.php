...
<div class="min-h-screen bg-gradient-to-br from-gray-50 via-green-50 to-emerald-50 p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
            ...
            <div class="text-white">
                <p class="text-4xl font-black mb-1">{{ $products->filter(function($product) { 
        ...
        <div class="bg-gradient-to-br from-yellow-500 to-orange-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
            ...
            <div class="text-white">
                <p class="text-4xl font-black mb-1">{{ $products->where('status', 'active')->count() }}</p>
                        ...
                        <td class="px-4 py-4 text-center">
                            @if($product->approved_at)
                                @php
                                    $approvedDate = is_string($product->approved_at) ? \Carbon\Carbon::parse($product->approved_at) : $product->approved_at;
                                @endphp
                                <div class="text-sm font-medium text-gray-900">{{ $approvedDate->format('M d, Y') }}</div>
                                <div class="text-xs text-gray-500">{{ $approvedDate->diffForHumans() }}</div>
                            @else
        ...
        <div class="bg-gradient-to-r from-gray-50 to-slate-50 px-6 py-4 border-t border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="dataTables_info text-sm font-medium text-gray-700">
                    Showing <span class="font-bold text-green-600">{{ $products->firstItem() ?? 0 }}</span> to <span class="font-bold text-green-600">{{ $products->lastItem() ?? 0 }}</span> of <span class="font-bold text-green-600">{{ $products->total() }}</span> entries
                </div>
