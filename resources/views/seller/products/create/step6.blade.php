...
<form id="stepForm" action="{{ route('seller.products.create.step5.process') }}" method="POST">
    @csrf
...
function autoGenerateSEO() {
    // Data from server
    const productName = @json($productData['name'] ?? '');
    const productDesc = @json($productData['description'] ?? '');
    const brandName = @json($brand->name ?? '');
    const categoryName = @json($category->name ?? '');
    const price = @json($productData['price'] ?? '');
    ...
    const productSlug = @json($productData['slug'] ?? 'product');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('prevBtn')?.addEventListener('click', function() {
        window.location.href = '{{ route("seller.products.create.step5") }}';
    });
});
</script>
@endpush
