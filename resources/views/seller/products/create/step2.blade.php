...
<form id="stepForm" action="{{ route('seller.products.create.step2.process') }}" method="POST">
    @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const existingMainImageEl = document.getElementById('mainImageUrl');
    if (existingMainImageEl) {
        const existingMainImage = existingMainImageEl.value;
    ...
    // Load existing video
    const existingVideoEl = document.getElementById('videoUrl');
    if (existingVideoEl) {
        const existingVideo = existingVideoEl.value;
        if (existingVideo) {
...
function showMainImageLoading() {
    ...
    placeholder.innerHTML = `
        ...
        <div class="text-center">
            <p class="text-lg font-semibold text-blue-600">Uploading to ImageKit...</p>
...
['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => {
    ...
    if (additionalImagesUpload) {
        additionalImagesUpload.addEventListener(eventName, preventDefaults, false);
    }
});
...
['dragenter', 'dragover'].forEach(eventName => {
    ...
    if (mainImageUpload) {
        mainImageUpload.addEventListener(eventName, highlight, false);
    }
    if (additionalImagesUpload) {
        additionalImagesUpload.addEventListener(eventName, highlight, false);
    }
});
...
['dragleave', 'drop'].forEach(eventName => {
    ...
    if (mainImageUpload) {
        mainImageUpload.addEventListener(eventName, unhighlight, false);
    }
    if (additionalImagesUpload) {
        additionalImagesUpload.addEventListener(eventName, unhighlight, false);
    }
});
...
const mainImageUploadDrop = document.getElementById('mainImageUpload');
const additionalImagesUploadDrop = document.getElementById('additionalImagesUpload');

if (mainImageUploadDrop) {
    mainImageUploadDrop.addEventListener('drop', handleMainImageDrop, false);
}
if (additionalImagesUploadDrop) {
    additionalImagesUploadDrop.addEventListener('drop', handleAdditionalImagesDrop, false);
}
