...
<form id="stepForm" action="{{ route('seller.products.create.step2.process') }}" method="POST" enctype="multipart/form-data">
    @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @if(old('variants_data'))
        try {
            const oldVariants = JSON.parse('{!! addslashes(old('variants_data')) !!}');
    ...
    @elseif(session('seller_product_data.variants_data'))
        try {
            const sessionVariants = @json(json_decode(session('seller_product_data.variants_data'), true));
