...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-all border-2 border-red-200">
            ...
            <!-- Product Info -->
            <div class="p-4">
                <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ Str::limit($product->name, 40) }}</h3>
                <p class="text-sm text-gray-500 mb-3">{{ $product->category->name ?? 'N/A' }}</p>
                ...
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <span class="text-xl font-bold text-gray-900">₹{{ number_format($product->sale_price ?? $product->price, 2) }}</span>
                ...
                <div class="text-xs text-gray-500 mb-3">
                    Rejected: {{ $product->updated_at->diffForHumans() }}
                </div>
