        ...
        <div class="overflow-x-auto">
            <table id="tagsTable" class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($tags as $tag)
                    <tr class="group hover:bg-gradient-to-r hover:from-indigo-50 hover:to-indigo-50 transition-all duration-200" data-id="{{ $tag->id }}" data-status="{{ $tag->is_active ? '1' : '0' }}">
                        
                        <!-- Checkbox -->
                        <td class="px-4 py-4 text-center">
                            <input type="checkbox" class="row-select w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-2 focus:ring-purple-500" value="{{ $tag->id }}">
                        </td>
        ...
        <div class="bg-gradient-to-r from-gray-50 to-slate-50 px-6 py-4 border-t border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="dataTables_info text-sm font-medium text-gray-700">
                    Showing <span class="font-bold text-indigo-600">1</span> to <span class="font-bold text-indigo-600">{{ count($tags) }}</span> of <span class="font-bold text-indigo-600">{{ count($tags) }}</span> entries
                </div>
...
<div id="tagModal" class="fixed inset-0 z-50 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all duration-300 ease-out animate-slideUp sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
            <form id="tagForm" method="POST" action="{{ route('seller.tags.store') }}">
                @csrf
