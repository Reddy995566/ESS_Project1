        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-indigo-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
                ...
                <div class="text-white">
                    <p class="text-4xl font-black mb-1">{{ $totalNotifications ?? 0 }}</p>
            ...
            <div class="bg-orange-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
                ...
                <div class="text-white">
                    <p class="text-4xl font-black mb-1">{{ $unreadCount ?? 0 }}</p>
            ...
            <div class="bg-emerald-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
                ...
                <div class="text-white">
                    <p class="text-4xl font-black mb-1">{{ $readCount ?? 0 }}</p>
            ...
            <div class="bg-purple-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
                ...
                <div class="text-white">
                    <p class="text-4xl font-black mb-1">{{ $todayCount ?? 0 }}</p>
                ...
                <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="md:col-span-1">
                        ...
                        <select name="type" class="block w-full px-4 py-3 bg-white border-2 border-gray-300 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                            ...
                            <option value="order_placed" {{ request('type') == 'order_placed' ? 'selected' : '' }}>🛒 New Order</option>
                            <option value="product_approved" {{ request('type') == 'product_approved' ? 'selected' : '' }}>✅ Product Approved</option>
                            <option value="product_rejected" {{ request('type') == 'product_rejected' ? 'selected' : '' }}>❌ Product Rejected</option>
                            <option value="payout_processed" {{ request('type') == 'payout_processed' ? 'selected' : '' }}>💰 Payout Processed</option>
                            <option value="low_stock" {{ request('type') == 'low_stock' ? 'selected' : '' }}>⚠️ Low Stock</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <a href="{{ route('seller.notifications.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-3 rounded-xl font-semibold transition-all">
                                Reset
                            </a>
                        </div>
                    </div>
            ...
            <div class="overflow-x-auto">
                <table id="notificationsTable" class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr class="group hover:bg-gradient-to-r hover:from-indigo-50 hover:to-blue-50 transition-all duration-200 {{ ($notification->read_at || $notification->is_read) ? '' : 'bg-blue-50' }}"
                            data-type="{{ $notification->type }}"
                            data-read="{{ ($notification->read_at || $notification->is_read) ? 'read' : 'unread' }}"
                            data-id="{{ $notification->id }}">
                            
                            <!-- Checkbox -->
                            <td class="px-4 py-4 text-center">
                                <input type="checkbox" class="row-select w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-2 focus:ring-indigo-500" value="{{ $notification->id }}">
                            </td>
                            ...
                            <td class="px-4 py-4">
                                ...
                                @endphp
                                <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-bold bg-gradient-to-r {{ $typeData['color'] }} border">
                                    {{ $typeData['icon'] }}
                                </span>
                            </td>
                            ...
                            <!-- Title -->
                            <td class="px-4 py-4">
                                <p class="text-sm font-bold text-gray-900">{{ $notification->title ?? ucfirst(str_replace('_', ' ', $notification->type)) }}</p>
                            ...
                            <!-- Message -->
                            <td class="px-4 py-4">
                                <p class="text-sm text-gray-600">{{ Str::limit($notification->message ?? 'No message available', 100) }}</p>
                            </td>
                            ...
                            <!-- Date -->
                            <td class="px-4 py-4 text-center">
                                <p class="text-sm font-medium text-gray-900">{{ $notification->created_at->format('d M, Y') }}</p>
                                <p class="text-xs text-gray-500">{{ $notification->created_at->format('h:i A') }}</p>
                            </td>
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center justify-center space-x-2">
                                    @if(!$notification->read_at && !$notification->is_read)
                                        <button class="mark-read-btn inline-flex items-center justify-center w-9 h-9 text-green-600 bg-green-50 border-2 border-green-200 rounded-lg hover:bg-green-100 hover:border-green-400 focus:outline-none focus:ring-2 focus:ring-green-500 transition-all" title="Mark as Read" data-id="{{ $notification->id }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                                            </svg>
                                        </button>
                                    @endif
                                    <button class="delete-btn inline-flex items-center justify-center w-9 h-9 text-red-600 bg-red-50 border-2 border-red-200 rounded-lg hover:bg-red-100 hover:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-500 transition-all" title="Delete Notification" data-id="{{ $notification->id }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-gradient-to-r from-gray-50 to-slate-50 px-6 py-4 border-t border-gray-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="dataTables_info text-sm font-medium text-gray-700">
                        Showing <span class="font-bold text-indigo-600">{{ $notifications->firstItem() ?? 0 }}</span> to <span class="font-bold text-indigo-600">{{ $notifications->lastItem() ?? 0 }}</span> of <span class="font-bold text-indigo-600">{{ $notifications->total() }}</span> entries
                    </div>
