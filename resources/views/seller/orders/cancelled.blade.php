        ...
        <div class="mb-8">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                <div class="flex items-center space-x-3">
                    <a href="{{ route('seller.orders.index') }}" class="inline-flex items-center px-5 py-2.5 bg-white text-gray-700 text-sm font-semibold rounded-xl border-2 border-gray-200 hover:border-indigo-400 hover:bg-indigo-50 hover:text-indigo-700 transition-all duration-200 shadow-sm hover:shadow-md">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-red-600 rounded-2xl shadow-xl p-6">
                ...
                <div class="text-white">
                    <p class="text-4xl font-black mb-1">{{ $orders->total() }}</p>
            ...
            <div class="bg-orange-600 rounded-2xl shadow-xl p-6">
                ...
                <div class="text-white">
                    <p class="text-4xl font-black mb-1">{{ $orders->where('cancelled_by', 'customer')->count() }}</p>
                    <p class="text-orange-100 text-sm font-medium">Customer Cancelled</p>
                </div>
            </div>

            <div class="bg-indigo-600 rounded-2xl shadow-xl p-6">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center justify-center w-12 h-12 bg-white bg-opacity-20 rounded-xl backdrop-blur-sm">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span class="px-3 py-1 bg-white bg-opacity-20 backdrop-blur-sm rounded-full text-xs font-bold text-white">By Seller</span>
                </div>
                <div class="text-white">
                    <p class="text-4xl font-black mb-1">{{ $orders->where('cancelled_by', 'seller')->count() }}</p>
                    <p class="text-indigo-100 text-sm font-medium">Seller Cancelled</p>
                </div>
            </div>
        </div>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="group hover:bg-gradient-to-r hover:from-red-50 hover:to-pink-50 transition-all duration-200">
                            ...
                            <td class="px-4 py-4">
                                <p class="text-sm font-medium text-gray-900">{{ $order->first_name }} {{ $order->last_name }}</p>
                                <p class="text-xs text-gray-500">{{ $order->email }}</p>
                            </td>
                            <td class="px-4 py-4 text-center">
                                <p class="text-sm font-bold text-gray-900">₹{{ number_format($order->items->sum('seller_amount'), 2) }}</p>
                            </td>
                            <td class="px-4 py-4 text-center">
                                <span class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold 
                                    @if($order->cancelled_by === 'customer') bg-orange-100 text-orange-700 border border-orange-300
                                    @elseif($order->cancelled_by === 'seller') bg-indigo-100 text-indigo-700 border border-indigo-300
                                    @else bg-purple-100 text-purple-700 border border-purple-300
                                    @endif">
                                    {{ ucfirst($order->cancelled_by ?? 'N/A') }}
                                </span>
                            </td>
                            <td class="px-4 py-4 text-center">
                                <p class="text-sm text-gray-900">{{ $order->cancelled_at ? $order->cancelled_at->format('d M, Y') : 'N/A' }}</p>
                                <p class="text-xs text-gray-500">{{ $order->cancelled_at ? $order->cancelled_at->format('h:i A') : '' }}</p>
                            </td>
                            <td class="px-4 py-4">
                                <p class="text-xs text-gray-600 line-clamp-2">{{ $order->cancellation_reason ?? 'No reason provided' }}</p>
                            </td>
                            <td class="px-4 py-4 text-center">
                                <a href="{{ route('seller.orders.show', $order->id) }}" class="inline-flex items-center px-3 py-1.5 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white text-xs font-semibold rounded-lg hover:from-indigo-600 hover:to-indigo-700 shadow-md hover:shadow-lg transition-all">
