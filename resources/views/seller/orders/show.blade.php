        ...
        <div class="mb-8">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="flex items-center space-x-4">
                    <a href="{{ route('seller.orders.index') }}" class="flex items-center justify-center w-12 h-12 bg-white rounded-xl shadow-md hover:shadow-lg transition-all">
                ...
                <div class="flex items-center space-x-3">
                    @if(!$order->shiprocket_order_id)
                        @if($shiprocketEnabled && $hasCredentials)
                    ...
                    <a href="{{ route('seller.orders.invoice', $order->id) }}" target="_blank" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-green-600 to-green-700 text-white text-sm font-semibold rounded-xl hover:from-green-700 hover:to-green-800 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
                    <div class="bg-gradient-to-r from-gray-50 to-slate-50 px-6 py-4 border-b border-gray-200">
                        <h2 class="text-xl font-bold text-gray-900 flex items-center">
                            ...
                            Your Items in this Order ({{ $order->items->where('seller_id', auth('seller')->id())->count() }})
                        </h2>
                    </div>
                    <div class="p-6">
                        <div class="space-y-4">
                            @foreach($order->items->where('seller_id', auth('seller')->id()) as $item)
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center gap-4 mt-2">
                                        <span class="text-xs text-red-600 font-semibold">
                                            Commission: -₹{{ number_format($item->commission_amount ?? 0, 2) }} ({{ $item->commission_rate ?? 0 }}%)
                                        </span>
                                        <span class="text-xs text-green-600 font-bold">
                                            Your Earning: ₹{{ number_format($item->seller_amount ?? 0, 2) }}
                                        </span>
                                    </div>
                                </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-semibold text-gray-900">₹{{ number_format($totalSales, 2) }}</span>
                            </div>
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-600">Platform Commission</span>
                                <span class="font-semibold text-red-600">-₹{{ number_format($totalCommission, 2) }}</span>
                            </div>
                            <div class="border-t-2 border-gray-200 pt-3 mt-3">
                                <div class="flex justify-between">
                                    <span class="text-base font-bold text-gray-900">Your Earnings</span>
                                    <span class="text-2xl font-black text-green-600">₹{{ number_format($totalEarnings, 2) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    const shiprocketOrderUrl = "{{ route('seller.orders.shiprocket', $order->id) }}";
    const shiprocketCouriersUrl = "{{ route('seller.orders.shiprocket.couriers', $order->id) }}";
    const shiprocketAwbUrl = "{{ route('seller.orders.shiprocket.awb', $order->id) }}";
    ...
    function cancelOrder() {
        ...
        fetch('{{ route('seller.orders.cancel', $order->id) }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken
