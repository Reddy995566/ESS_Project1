...
@section('content')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-indigo-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
                ...
                <div class="text-white">
                    <p class="text-4xl font-black mb-1">{{ $totalCollections }}</p>
            ...
            <div class="bg-teal-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
                ...
                <div class="text-white">
                    <p class="text-4xl font-black mb-1">{{ $activeCollections }}</p>
            ...
            <div class="bg-pink-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
                ...
                <div class="text-white">
                    <p class="text-4xl font-black mb-1">{{ $homepageCollections }}</p>
                ...
                <table id="collectionsTable" class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr class="group hover:bg-gradient-to-r hover:from-indigo-50 hover:to-indigo-50 transition-all duration-200"
                            data-status="{{ $collection->is_active ? '1' : '0' }}"
                            data-homepage="{{ $collection->show_in_homepage ? '1' : '0' }}"
                            ...
                            <td class="px-4 py-4 text-center">
                                <div class="flex justify-center">
                                    @if($collection->imagekit_url || $collection->image)
                                        <img src="{{ $collection->imagekit_thumbnail_url ?: $collection->imagekit_url ?: $collection->image }}" alt="{{ $collection->name }}" class="w-12 h-12 rounded-lg object-cover shadow-md border-2 border-gray-300 group-hover:border-indigo-400 transition-all">
                                    @else
                            ...
                            <td class="px-4 py-4">
                                <div>
                                    <p class="text-sm font-bold text-gray-900 group-hover:text-indigo-700">{{ $collection->name }}</p>
                                    <p class="text-xs text-gray-500 mt-1">{{ $collection->slug }}</p>
                                </div>
                            </td>

                            <!-- Description -->
                            <td class="px-4 py-4">
                                <p class="text-sm text-gray-700 line-clamp-2">{{ Str::limit($collection->description, 100) ?: 'No description' }}</p>
                            </td>
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center justify-center space-x-2">
                                    <button class="edit-btn inline-flex items-center justify-center w-9 h-9 text-indigo-600 bg-indigo-50 border-2 border-indigo-200 rounded-lg hover:bg-indigo-100 hover:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                                            ...
                                            data-is-active="{{ $collection->is_active ? 'true' : 'false' }}"
                                            data-show-in-homepage="{{ $collection->show_in_homepage ? 'true' : 'false' }}">
            ...
            <div class="bg-gradient-to-r from-gray-50 to-slate-50 px-6 py-4 border-t border-gray-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="dataTables_info text-sm font-medium text-gray-700">
                        Showing <span class="font-bold text-indigo-600">1</span> to <span class="font-bold text-indigo-600">{{ count($collections) }}</span> of <span class="font-bold text-indigo-600">{{ count($collections) }}</span> entries
                    </div>
    ...
    <div id="collectionModal" class="fixed inset-0 z-50 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all duration-300 ease-out animate-slideUp sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full">
                <form id="collectionForm" method="POST" action="{{ route('seller.collections.store') }}">
                    @csrf
    ...
    @include('seller.collections.scripts')
@endsection
